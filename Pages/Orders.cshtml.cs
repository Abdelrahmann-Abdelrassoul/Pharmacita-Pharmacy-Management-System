        ...
        public IActionResult OnPost()
        {
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    // Validate foreign key for Customer
                    ...
                    // Decrement Quantity_IN_Stock for the selected product
                    string productsql = "UPDATE Products SET Quantity_IN_Stock = Quantity_IN_Stock - 1 WHERE Product_ID = @P_ID";
                    using (SqlCommand command = new SqlCommand(productsql, connection))
                    {
                        command.Parameters.AddWithValue("@P_ID", NewOrder.P_ID);
                        command.ExecuteNonQuery();
                    }
